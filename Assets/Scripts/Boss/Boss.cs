...
public class Boss : MonoBehaviour, IDamageable
{
    ...
    //Decrease gap on iterations
    public float decreaseValue = 0.3f;
    public float minGapValue = 1.5f;

    [HideInInspector] public bool justStunned = false;
    ...
    public IEnumerator triggerArrows()
    {
        ...
        for (int i = 0; i < arrowList.Count; i++)
        {
            if (warningList[i].gameObject.activeSelf)
            {
                arrowList[i].transform.position = new Vector2(warningList[i].transform.position.x, transform.position.y + 4f);
